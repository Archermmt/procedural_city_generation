...
class GUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        from procedural_city_generation.additional_stuff.IOHelper import (
            StdoutRedirector,
        )
    ...
    def createTable(self, submodule):
        ...
        from procedural_city_generation.additional_stuff.Param import (
            paramsFromJson,
            jsonFromParams,
        )
        ...
        for parameter in params:
            ...
            s = (
                ""
                if parameter.value_lower_bound is None
                else str(parameter.value_lower_bound)
            )
            ...
            s = (
                ""
                if parameter.value_upper_bound is None
                else str(parameter.value_upper_bound)
            )
            g = QtWidgets.QTableWidgetItem(s)
            g.setFlags(g.flags() & ~QtCore.Qt.ItemIsEditable)
            g.setBackground(QtGui.QBrush(QtGui.QColor(235, 235, 235)))
    ...
    def clean_directories(self):
        from procedural_city_generation.additional_stuff.clean_tools import (
            clean_pyc_files,
        )
